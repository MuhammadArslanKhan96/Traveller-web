...
const Flight = ({ item }: FlightProp) => {
    ...
    React.useEffect(() => {
        setActive({ id: item[0].id, image: item[0].image })
    }, [item])
    ...
    let flight = item.filter((i: any) => (i.id === active.id || i.image === active.image))[0];
    ...
    return (
        <div className='flex items-center max-lg:flex-col gap-9'>
            <div className="flex items-center lg:max-w-[600px] max-sm:flex-wrap max-sm:justify-center">
                ...
                {item.length > 1 && item.filter((i: any) => i.id !== active.id).map((item: any, idx: number) => (
                    <div key={idx} onClick={() => setActive({ id: item.id, image: item.image })} className="drop-shadow-[0px_12px_24px_rgba(0,0,0,0.07)] cursor-pointer relative">
                        <div className='flex absolute top-8 left-0 py-1 px-2 justify-center items-center gap-2.5 rounded-tr-xl bg-primary'>
            ...
            <div className='flex flex-col gap-8 max-lg:text-center lg:max-w-[400px]'>
                <div className="flex flex-col gap-5">
                    <h2 className='text-5xl font-bold text-dark'>{flight && flight['company-name']}</h2>
